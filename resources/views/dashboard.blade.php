        ...
        <div class="bg-white/10 border border-white/20 shadow-2xl rounded-xl p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 ">
                ...
                <x-card title="Faturamento" class="bg-green-500/20! border-green-500/30! hover:bg-green-500/30!">
                    <p class="text-xl sm:text-2xl font-bold text-white">{{ $totalReceitaUSD }} <span class="text-white/60 text-sm">USD</span></p>
                    <p class="text-xl sm:text-2xl font-bold text-white">{{ $totalReceitaBRL }} <span class="text-white/60 text-sm">BRL</span></p>
                </x-card>

                {{-- Total de Reembolsos USD e BRL --}}
                <x-card title="Total de Reembolsos" class="bg-red-500/20! border-red-500/30! hover:bg-red-500/30!">
                    <p class="text-xl sm:text-2xl font-bold text-white">{{ $totalReembolsosUSD }} <span class="text-white/60 text-sm">USD</span></p>
                    <p class="text-xl sm:text-2xl font-bold text-white">{{ $totalReembolsosBRL }} <span class="text-white/60 text-sm">BRL</span></p>
                </x-card>

                {{-- Receita Líquida USD e BRL --}}
                <x-card title="Receita Líquida" class="bg-blue-500/20! border-blue-500/30! hover:bg-blue-500/30!">
                    <p class="text-xl sm:text-2xl font-bold {{ $receitaLiquidaUSD > 0 ? 'text-green-400' : 'text-red-400' }}">{{ $receitaLiquidaUSD }} <span class="text-white/60 text-sm">USD</span></p>
                    <p class="text-xl sm:text-2xl font-bold {{ $receitaLiquidaUSD > 0 ? 'text-green-400' : 'text-red-400' }}">{{ $receitaLiquidaBRL }} <span class="text-white/60 text-sm">BRL</span></p>
                </x-card>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
            ...
            <x-card title="Pedidos Entregues" class="bg-yellow-500/20! border-yellow-500/30! hover:bg-yellow-500/30!">
                ...
                <div class="flex items-center gap-2 mt-2">
                    <span class="text-lg font-semibold {{ $taxaPedidosEntregues >= 80 ? 'text-green-400' : ($taxaPedidosEntregues >= 50 ? 'text-yellow-400' : 'text-red-400') }}">
            ...
            <x-card title="Taxa de Reembolso" class="bg-orange-500/20! border-orange-500/30! hover:bg-orange-500/30!">
                ...
                <div class="flex items-center gap-2 mt-2">
                    <span class="text-lg font-semibold {{ $taxaReembolso <= 5 ? 'text-green-400' : ($taxaReembolso <= 15 ? 'text-yellow-400' : 'text-red-400') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-4">
                        <div class="bg-white/10 rounded-lg p-4 border border-white/20">
                            ...
                            <p class="text-2xl sm:text-3xl font-bold text-white">{{ $produtoMaisVendido['quantity']}}</p>
                        </div>
                        <div class="bg-white/10 rounded-lg p-4 border border-white/20">
            ...
            <div class="bg-green-500/20 border-2 border-green-400/40 shadow-2xl rounded-xl p-6 backdrop-blur-md">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-4">
                        <div class="bg-white/10 rounded-lg p-4 border border-white/20">
                            <p class="text-sm text-white/60 mb-1">Quantidade Vendida</p>
                            <p class="text-2xl sm:text-3xl font-bold text-white">{{ $produtoMaisFaturado['quantity']}}</p>
                        </div>
                        <div class="bg-white/10 rounded-lg p-4 border border-white/20">
                            <p class="text-sm text-white/60 mb-1">Receita Gerada</p>
                            <p class="text-xl sm:text-2xl font-bold text-green-400">
